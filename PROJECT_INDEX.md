...
- The desktop renderer now owns a frameless title area and custom window controls. `AppChrome`, `AppSidebar`, and `WindowControls` provide the floating Library/Albums shell, sidebar settings popover, and macOS-inspired glass layout; the global nested Tailwind drop-shadow filter is removed.
